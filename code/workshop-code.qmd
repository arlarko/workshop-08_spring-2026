...
drought_exp_clean <- drought_exp |>
  # remove spaces (make underscore) make undercase
  clean_names() |>
  # expand code names to full species names, creating new colum called species_name
  ...
  # expand code names to full treatment name, making new column 
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |>
  # changing order of each scientific name and species level 
...
ggplot(data = drought_exp_clean,
  ...
  #geom for scatterplot
  geom_point() +
  #relabeling x, y and title
  labs(x = "Specific leaf area (cm\U00B2 / g)",
       y = "Total plant mass (g)",
       title = "As SLA inclreases total mass also increases")
...
ggplot(data = drought_exp_clean,
       # x = treatment
       # y = total plant mass
       # color geom by treatment
       mapping = aes(
         x = water_treatment,
         y = total_g,
         color = water_treatment
       )) +
  #jitter to represent individual plats
  geom_jitter(
    width = 0.1,
    height = 0
  )+
...
model4 <- lm(
  total_g ~ water_treatment + species_name, #formula: only water treatment and species
  data = drought_exp_clean
)
...
model6 <- lm(
  total_g ~  water_treatment, #formula: only water treat
  data = drought_exp_clean
)
...
# function dredge fits model automatically, no thinking, doesnt help with model selection, dont know why each one would exist, doesnt consider biological interaction, not to use in this class (package may be newman)
```
...
- no clear outliers in data set influencing
...
# gives measure of a model that takes into factors in model and weights against complexity (# of factors in model), trying to get the model that best describes the differences
...
AICc(
  ...
  #arrange order of aic
  arrange(AICc)

#model 4 best model, only includes species and treatment 
```
...
- `(intercept)`: model prediction for plant mass for Lotus scoparius in the drought stressed treatment (lotus reference level for species and drought reference level for water treatment, dont show up in r output, how you know which the reference are)
- `water_treatmentWell Watered` : model prediction for the difference in lotus scoparious mass between the well watered and drought stressed treatment
- `species_namePenstemon centranthifolius` : model prediction for difference between penstemon mass in the well watered treatment and Lotus mass in well watered treatment
- `species_nameEschscholzia californica`: model prediction for the difference between Escholtiza in the well watered treatment and lotus mass in well watered treatment 

## Stop and think: what does this model mean?  

**What is the best model?**

the best model includes water treatement and species as predictors

**How much variation in the response (total mass, in grams) does this model explain?**

72.6% (from Adjusted R-squared:  0.7257) of the variation in plant mass is explained by a model with both of these predictors.
...
preds <- ggpredict(model4,
  ...
  rename(water_treatment = x,
         species_name = group) #rename column to fit original data frame 
```
