...
drought_exp_clean <- drought_exp |>
  ...
  clean_names() |>
  # expand code names to full species names, creating new colum called
  #  species_name
...
model4 <- lm(
  total_g ~ water_treatment + species_name, #formula:water treatment & species
  data = drought_exp_clean
)
...
# function dredge fits model automatically, no thinking, doesnt help with model
# selection, dont know why each one would exist, doesnt consider
#  biological interaction, not to use in this class 
#  (package may be newman)
```
...
# gives measure of a model that takes into factors in model and weights
#against complexity (# of factors in model), trying to get the model that
